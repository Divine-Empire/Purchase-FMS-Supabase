...
export default function Verification() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [dataRes, dropdownRes] = await Promise.all([
        fetch(`/api/verification?${params.toString()}`),
        fetch("/api/dropdowns"),
      ]);
      ...
      if (dataJson.success) {
        ...
        toast.error(dataJson.error || "Failed to load verification records");
      }
    } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const records = selectedRecords.map((rec) => ({
        ...
        verificationDate: formData.verificationDate || new Date(),
  ...
  const isFormValid = formData.checkedBy && formData.verificationDate;
    ...
    <div className="p-6 min-h-screen bg-[#f8fafc]">
          ...
          <TabsList className="bg-indigo-50/50 p-1 rounded-xl h-auto grid grid-cols-2 gap-1.5 border border-indigo-100/50 w-[420px] shadow-2xs">
            ...
            >
              ...
              <div className="flex flex-col items-start leading-none gap-1 text-left">
                ...
                <span className="text-[10px] opacity-70 font-medium">Awaiting verification</span>
              </div>
            ...
            >
              ...
              <div className="flex flex-col items-start leading-none gap-1 text-left">
                ...
                <span className="text-[10px] opacity-70 font-medium">Verified invoices</span>
              </div>
              <Badge variant="secondary" className={cn(
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              Verification by Accounts ({selectedRows.size} Selected)
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setFormData({ ...formData, checkedBy: v })}
                >
              ...
              <div className="space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="w-full text-left font-normal border-gray-300 shadow-sm"
                    >

...
export default function OrderCancelPage() {
  ...
  const fetchCancelledOrders = async () => {
    ...
    try {
      const response = await fetch(`/api/order-cancel?_t=${Date.now()}`)
      if (!response.ok) {
  ...
  const fetchDropdownItems = async () => {
    try {
      ...
      const json = await response.json()
      if (json.success && json.data && Array.isArray(json.data.dropdownData)) {
        const items = json.data.dropdownData
          .map((row: any) => String(row.itemName || "").trim())
          .filter(Boolean)
        const uniqueItems = Array.from(new Set(items)) as string[]
        setDropdownItemsList(uniqueItems)
      }
    } catch (err) {
      console.error("Error fetching items from dropdown API:", err)
    }
  }
  ...
  // Auto-populate cancelStage from selection
  useEffect(() => {
    if (selectedSearchRowIds.length > 0) {
      const firstSelected = searchResults.find(r => r.id === selectedSearchRowIds[0])
      if (firstSelected) {
        setCancelStage(firstSelected.currentPendingStage || "")
      }
    } else {
      setCancelStage("")
    }
  }, [selectedSearchRowIds, searchResults])

  // Purchaser-based record access: only show records this user is allowed to see.
  const visibleCancelledOrders = useMemo(
    () => cancelledOrders.filter((order) => canViewPurchaserRecord(order.purchaser, recordsAccess, role)),
    [cancelledOrders, recordsAccess, role]
  )

  // Filter orders based on search term
  const filteredCancelledOrders = useMemo(() => {
    if (!searchTerm) return visibleCancelledOrders

    return visibleCancelledOrders.filter((order) => {
      return (
  ...
  }, [visibleCancelledOrders, searchTerm])
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`/api/order-cancel/search?searchType=${searchType}&query=${encodeURIComponent(searchQuery)}`)
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
      }
                        ...
                        >
                          ...
                          <TableCell className="text-center py-1">
                            <Input
                              ...
                              className="w-20 text-center h-8 bg-slate-50 border-slate-200 text-slate-500 mx-auto"
                              disabled
                            />
                          </TableCell>
                        </TableRow>
              ...
              <div className="border border-red-100 bg-red-50/10 p-5 rounded-lg space-y-4 animate-in slide-in-from-top-4 duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="cancelStage">Order Cancel Stage (Auto-populated)</Label>
                    <Input
                      id="cancelStage"
                      value={
                        cancelStage === "indent-approval" ? "Stage 2: Indent Approval" :
                        cancelStage === "update-3-vendors" ? "Stage 3: Update 3 Vendors" :
                        cancelStage === "negotiation" ? "Stage 4: Negotiation" :
                        cancelStage === "po-entry" ? "Stage 5: PO Entry" :
                        cancelStage === "follow-up-vendor" ? "Stage 6: Follow-up Vendor" :
                        cancelStage === "transporter-follow-up" ? "Stage 6.1: Transporter Follow-Up" :
                        cancelStage || "—"
                      }

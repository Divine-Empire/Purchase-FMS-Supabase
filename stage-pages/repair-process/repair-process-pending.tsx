...
interface RepairProcessPendingProps {
  pending: any[];
...
export default function RepairProcessPending({
  pending,
  handleOpenForm,
  selectedPendingColumns,
  PENDING_COLUMNS,
  safeValue,
}: RepairProcessPendingProps) {
  if (pending.length === 0) {
    return (
      <div className="bg-white border rounded-xl p-12 text-center shadow-sm">
        ...
        <p className="text-slate-500 mt-1 max-w-xs mx-auto">
          Items rejected with reject type "Repair" will show up here.
        </p>
      </div>
    );
  }
        ...
        <table className="w-full text-sm text-left border-separate border-spacing-0 min-w-[1200px]">
          ...
          <tbody className="divide-y divide-slate-100">
            {pending.map((record: any) => (
              <tr
                key={record.id}
                className="even:bg-slate-50/30 hover:bg-amber-50/20 transition-colors border-b border-slate-100 last:border-0 text-center"
              >
                <td className="sticky left-0 z-10 bg-white group-hover:bg-amber-50/10 px-4 py-2 whitespace-nowrap border-b border-slate-100 text-center">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleOpenForm(record.id)}
                    className="h-8 bg-white hover:bg-amber-50 hover:text-amber-700 hover:border-amber-200 transition-all font-semibold px-3 text-xs shadow-2xs"
                  >
                    Process Repair
                  </Button>
                </td>

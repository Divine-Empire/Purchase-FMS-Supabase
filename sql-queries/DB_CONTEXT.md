...
### 2026-09-11 — `pfms_tat` rebuilt (snake_case, minutes, array of responsible persons)
- Old `pfms_tat` (`id text`, `stageName`, `actionTime` int-hours, `responsibleNames` comma-string) renamed to **`pfms_tat_old_backup`** (not dropped) and replaced with a fresh table:
  `id uuid PK default gen_random_uuid()`, `stage_name text UNIQUE`, `duration_in_minutes integer default 60`, `responsible_persons text[] default '{}'`, `created_at`, `updated_at`.
- All 18 stage rows reseeded with the **same `stage_name` values** the app already looks up (`indent-approval`, `update-3-vendors`, `negotiation`, `po-entry`, `follow-up-vendor`, `transporter-flw-up`, `material-received`, `serial-generation`, `receipt-in-tally`, `material-testing`, `purchase-return`, `return-approval`, `submit-invoice-ho`, `submit-invoice`, `acc-verification`, `vendor-payments`, `freight-payments`, `warranty-claim`) — every row defaults to **60 minutes**, `responsible_persons = '{}'` (old durations/assignees intentionally not carried over, per the requested reset).
- New dropdown category **`Responsible Person`** added to `pfms_dropdown` (empty for now — names to be seeded once supplied) as the source for the new multi-select checkbox UI in `dropdownsMaster.tsx`'s Edit modal.
- Code updated to match: `app/api/helper/plannedCalculator.ts` (now queries/operates in minutes directly, no more hours↔minutes conversion inside the working-hours-shift loop), `app/api/dropdowns/route.ts` (`upsertResponsible` now takes `durationMinutes` + `responsiblePersons: string[]`), `app/api/cron/generate-report/route.ts` (`respMap` built from `stage_name`/`responsible_persons` array), `app/api/vendor-payment/route.ts`.
- **Bug fixed in passing**: `app/api/vendor-payment/route.ts` was querying `stage_name = 'vendor-payment'` (singular) against a table that only ever had `'vendor-payments'` (plural) — that TAT lookup silently always missed and fell back to a hardcoded 72-hour default. Fixed to query the correct plural stage name.
- Frontend: `stage-pages/master/dropdownsMaster.tsx`'s "Edit Responsible Persons" modal redesigned — TAT is now three Days/Hours/Minutes number inputs (converted to/from total minutes via `lib/utils.ts`'s `dhmToMinutes`/`minutesToDHM`), and Responsible Person(s) is now a multi-select checkbox grid sourced from the `Responsible Person` dropdown category (was a free-text comma-separated textarea).

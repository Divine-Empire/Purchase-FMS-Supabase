...
interface RepairProcessHistoryProps {
  history: any[];
...
export default function RepairProcessHistory({
  history,
  selectedHistoryColumns,
  HISTORY_COLUMNS,
  safeValue,
}: RepairProcessHistoryProps) {
  if (history.length === 0) {
    return (
      <div className="bg-white border rounded-xl p-12 text-center shadow-sm">
        ...
        <p className="text-slate-500 mt-1">Completed repairs will appear here.</p>
      </div>
    );
  }
            ...
            {history.map((record: any) => (
              ...
              >
                {HISTORY_COLUMNS.filter((col) =>
                  ...
                  if (col.key === "delay") {
                    const pVal = record.data?.plan9 || record.data?.repairDate;
                    const aVal = record.data?.repairDate;
